...
class Game():
    ...
    def drawAguacate(self):
        x = self.aguacate.x * Config.CELLSIZE
        y = self.aguacate.y * Config.CELLSIZE
        aguacateRect = pygame.Rect(x, y, Config.CELLSIZE, Config.CELLSIZE)
        pygame.draw.rect(self.screen, Config.RED, aguacateRect)
    ...
    def displayGameOver(self):
        ...
        getRect.midtop = (Config.WINDOW_WIDTH / 2, 10)
    ...
    def gameLoop(self):
        while True:
            ...
            self.snake.update(self.aguacate)
            self.draw()
            if self.isGameOver():
                break
